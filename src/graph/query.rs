    ...
    if symbols.is_empty() {
        return Ok((format!(
            "No symbols found in file: {resolved_path}. The file may not contain indexable symbols.\n\
             parser_limitations: file was indexed but no symbols were extracted — the parser may not support this file's syntax patterns\n"
...
// Coverage and omission signals — reports parser confidence level and known blind spots per language
...
impl std::fmt::Display for CoverageLevel {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            CoverageLevel::Strong => write!(f, "strong"),
            CoverageLevel::Unknown => write!(f, "unknown"),
        }
    }
}

pub(crate) fn coverage_level_for_language(lang: Option<&str>) -> CoverageLevel {
    match lang {
        Some("rust" | "typescript" | "javascript" | "python" | "go" | "php") => CoverageLevel::Strong,
        _ => CoverageLevel::Unknown,
    }
}

pub(crate) fn omitted_hints_for_language(lang: Option<&str>) -> &'static [&'static str] {
    match lang {
        Some("rust") => &[
            "macro-generated code not resolved",
            "conditional compilation (cfg) branches not followed",
        ],
        Some("typescript") => &[
            "decorator-generated members not indexed",
            "computed property names not resolved",
        ],
        Some("javascript") => &[
            "decorator-generated members not indexed",
            "computed property names not resolved",
            "CommonJS dynamic requires not traced",
        ],
        Some("python") => &[
            "metaclass-generated attributes not indexed",
            "dynamic attribute assignment outside __init__ not captured",
        ],
        Some("go") => &[
            "code generated by go:generate not resolved",
            "interface satisfaction is structural — implicit impl edges not traced",
        ],
        Some("php") => &[
            "magic methods (__get/__set/__call) targets not resolved",
            "dynamic class instantiation not traced",
        ],
        _ => &[],
    }
}

pub(crate) fn format_coverage_suffix(level: &CoverageLevel, omitted: &[&str]) -> String {
    if omitted.is_empty() {
        format!(" | coverage: {level}")
    } else {
        format!(" | coverage: {level} | omitted: {}", omitted.join("; "))
    }
}
...
pub(crate) fn check_file_freshness(conn: &Connection, project_root: &Path, rel_path: &str) -> FileFreshness {
    let row: Option<(String, i64, Option<String>)> = conn
        .query_row(
            "SELECT blake3_hash, last_indexed_at, language FROM files WHERE path = ?1",
            params![rel_path],
    ...
    let Some((stored_hash, indexed_at, language)) = row else {
        return FileFreshness { status: FreshnessStatus::Unknown, indexed_at: None, language: None };
    };

    let abs_path = project_root.join(rel_path);
    let bytes = match std::fs::read(&abs_path) {
        Ok(b) => b,
        Err(_) => return FileFreshness { status: FreshnessStatus::Unknown, indexed_at: Some(indexed_at), language },
    };

    let current_hash = blake3::hash(&bytes).to_hex().to_string();
    if current_hash == stored_hash {
        FileFreshness { status: FreshnessStatus::Current, indexed_at: Some(indexed_at), language }
    } else {
        FileFreshness { status: FreshnessStatus::Stale, indexed_at: Some(indexed_at), language }
    }
}
...
pub(crate) fn check_files_freshness(conn: &Connection, project_root: &Path, paths: &HashSet<String>) -> FileFreshness {
    ...
    // Multi-file freshness doesn't carry a single language — each file may differ
    FileFreshness { status: overall_status, indexed_at: earliest_ts, language: None }
}
...
mod eval {
    ...
    fn format_freshness_footer_current() {
        let f = FileFreshness { status: FreshnessStatus::Current, indexed_at: Some(1711360200), language: None };
    ...
    fn format_freshness_footer_never() {
        let f = FileFreshness { status: FreshnessStatus::Unknown, indexed_at: None, language: None };
        let footer = format_freshness_footer(&f, FreshnessMode::SingleFile);
        assert_eq!(footer, "\n---\nfreshness: unknown | indexed_at: never\n");
    }

    // --- coverage and omission signals ---

    #[test]
    fn coverage_level_rust_is_strong() {
        assert_eq!(coverage_level_for_language(Some("rust")), CoverageLevel::Strong);
    }

    #[test]
    fn coverage_level_all_tier1_are_strong() {
        for lang in &["rust", "typescript", "javascript", "python", "go", "php"] {
            assert_eq!(coverage_level_for_language(Some(lang)), CoverageLevel::Strong, "expected Strong for {lang}");
        }
    }

    #[test]
    fn coverage_level_unknown_language_is_unknown() {
        assert_eq!(coverage_level_for_language(Some("haskell")), CoverageLevel::Unknown);
    }

    #[test]
    fn coverage_level_none_is_unknown() {
        assert_eq!(coverage_level_for_language(None), CoverageLevel::Unknown);
    }

    #[test]
    fn omitted_hints_rust_returns_known_gaps() {
        let hints = omitted_hints_for_language(Some("rust"));
        assert_eq!(hints.len(), 2);
        assert!(hints[0].contains("macro"));
        assert!(hints[1].contains("cfg"));
    }

    #[test]
    fn omitted_hints_unknown_returns_empty() {
        assert!(omitted_hints_for_language(Some("haskell")).is_empty());
        assert!(omitted_hints_for_language(None).is_empty());
    }

    #[test]
    fn format_coverage_suffix_with_omissions() {
        let suffix = format_coverage_suffix(&CoverageLevel::Strong, &["hint1", "hint2"]);
        assert_eq!(suffix, " | coverage: strong | omitted: hint1; hint2");
    }

    #[test]
    fn format_coverage_suffix_without_omissions() {
        let suffix = format_coverage_suffix(&CoverageLevel::Unknown, &[]);
        assert_eq!(suffix, " | coverage: unknown");
    }
